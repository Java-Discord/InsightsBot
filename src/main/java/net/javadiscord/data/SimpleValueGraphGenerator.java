...
public class SimpleValueGraphGenerator {
	...
	public Optional<byte[]> generateTimeSeries(long guildId, LocalDate start, LocalDate end, String sqlScript, String title, String... seriesTitles) {
		var results = this.getData(guildId, start, end, sqlScript, seriesTitles.length);
		if (results.isEmpty()) {
	...
	private Map<LocalDate, List<Number>> getData(long guildId, LocalDate start, LocalDate end, String sqlScript, int valuesCount) {
